...
def evaluate_hierarchical(X, true_labels, n_clusters):
    ...
    pred_labels = hierarchical.fit_predict(X)
    results = {
...
def plot_metrics(metrics, n_clusters_range):
    ...
    plt.tight_layout()
    plt.savefig("C:/Users/lenovo/Desktop/ML Project/ML_Project_Group12/Evaluate/Hierarchical/evaluation.png", bbox_inches='tight', dpi=300)
    plt.show()
...
def find_optimal_clusters(metrics, n_clusters_range):
    ...
    for i, n in enumerate(n_clusters_range):
        ...
        scores[i] = score
    optimal_n_hierarchical = n_clusters_range[np.argmax(scores)]
    return optimal_n_hierarchical, scores

optimal_n_hierarchical, cluster_scores = find_optimal_clusters(metrics, n_clusters_range)

plt.figure(figsize=(10, 6))
plt.plot(n_clusters_range, cluster_scores, 'b-o', linewidth=2)
plt.axvline(x=optimal_n_hierarchical, color='r', linestyle='--', label=f'best: {optimal_n_hierarchical}')
plt.title('Hierarchical', fontsize=14)
plt.xlabel('number of clusters', fontsize=12)
...
plt.grid(True, alpha=0.3)
plt.legend()
plt.savefig("C:/Users/lenovo/Desktop/ML Project/ML_Project_Group12/Evaluate/Hierarchical/score.png", bbox_inches='tight', dpi=300)
plt.show()
print(f"Best: {optimal_n_hierarchical}")
...
def optimize_hierarchical_clusters(X, y_true, max_clusters=15):
    ...
    optimal_n_hierarchical, _ = find_optimal_clusters(metrics, n_clusters_range)
    return optimal_n_hierarchical, metrics

optimal_n_hierarchical, metrics = optimize_hierarchical_clusters(X, y_true, max_clusters=14)
