...
def evaluate_gmm(X, true_labels, n_components):
...
for n in n_clusters_range:
    scores = evaluate_gmm(X, y_true, n)
    for metric in metrics:
        metrics[metric].append(scores[metric])

def plot_metrics(metrics, n_clusters_range, save_path = None):
...
def find_optimal_clusters(metrics, n_clusters_range):
    ...
    for i, n in enumerate(n_clusters_range):
        ...
        scores[i] = score
    optimal_n_gmm = n_clusters_range[np.argmax(scores)]
    return optimal_n_gmm, scores

optimal_n_gmm, cluster_scores = find_optimal_clusters(metrics, n_clusters_range)

plt.figure(figsize=(10, 6))
plt.plot(n_clusters_range, cluster_scores, 'b-o', linewidth=2)
plt.axvline(x=optimal_n_gmm, color='r', linestyle='--', label=f'best: {optimal_n_gmm}')
plt.title('GMM', fontsize=14)
plt.xlabel('number of clusters', fontsize=12)
...
plt.grid(True, alpha=0.3)
plt.legend()
plt.savefig("C:/Users/lenovo/Desktop/ML Project/ML_Project_Group12/Evaluate/GMM", bbox_inches='tight', dpi=300)
plt.show()
print(f"Best: {optimal_n_gmm}")



def optimize_gmm_clusters(X, y_true, max_clusters=15):
    ...
    plot_metrics(metrics, n_clusters_range, save_path = "C:/Users/lenovo/Desktop/ML Project/ML_Project_Group12/Evaluate/GMM")
    optimal_n_gmm, _ = find_optimal_clusters(metrics, n_clusters_range)
    return optimal_n_gmm, metrics

optimal_n_gmm, metrics = optimize_gmm_clusters(X, y_true, max_clusters=14)
